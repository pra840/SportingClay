        ...
        success: function(resultData) {
            if(resultData){
                for (var i = 0; i < resultData.scoreInfoBeanList.length; i++) {
                    $('#myScores').append("<p id='scoreById'><a href='http://localhost:8072/prama/sportingclay/shooter/score/"+resultData.scoreInfoBeanList[i].id+"/'>"+getScoreCardTime(resultData.scoreInfoBeanList[i].gameDate)+"</a></p>");
                }
            }
        },
        error: function(){
